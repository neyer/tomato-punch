...
class Punch(object):
    ...
    def parse_config(self):
        ...
        try:
            paths = [ getenv("HOME"), ".", os.path.join(getenv("HOME"),".todo")]
            files = [ "todo.cfg", ".todo.cfg", "config" ]
    ...
    def add_literal_line(self,line):
        ...
        self.close_punch_file()
        if not self.optlist.no_status:
            print "Starting pomodoro: " + line
        else:
            print line
    ...
    def add_out_line(self):
        ...
        self.close_punch_file()
        if not self.optlist.no_status: 
            print "Stopping pomodoro: " + lastrec[0]  
        else:
            print lastrec[0]
    ...
    def execute_in(self):
        ...
        if( len(self.args) == 1 ):
            last_record = self.get_last_punch_rec()
            last_task = last_record[0]
            self.add_literal_line(last_task)
            return
    ...
    def execute_genmon(self):
        self.parse_config()
        lastrec = self.get_last_punch_rec()
        if (len (lastrec) == 2):
            format_string = ""
            duration = time.time() -  time.mktime(time.strptime(lastrec[1],self.timestampFormat))
            pomodoro_length = 20*60
            fraction_finished = min(duration/pomodoro_length,1.0)
            percent_finished = 100*fraction_finished
            seconds_left = int(pomodoro_length - duration)
            minutes_left = int(seconds_left/60)
            seconds = (seconds_left - minutes_left*60)
            image_number = int(fraction_finished*ImageCount)
            
            genmon_result = "<img>%s/tomato-%d.png</img>"\
                            "<bar>%0.3f</bar>"\
                            "<txt>%#02d:%#02d</txt>"\
                            "<tool>%s</tool>"  \
                            "<click>/home/mneyer/bin/pomodoro.py --stop</click>"
            print genmon_result % (ImageDir, image_number, percent_finished,minutes_left,seconds,lastrec[0])
        else:
            print "<img>%s/tomato-green.png</img><txt>idle</txt><click>/home/mneyer/bin/pomodoro.py --start</click>" % ImageDir
    ...
    def execute_wh(self):
        ...
        if( len(self.args) == 1 ):
            lastrec = self.get_last_punch_rec()
            if ( len(lastrec) == 2 ):
                duration = " %s" % self.get_duration(lastrec[1], self.get_time())
                status = "Current task: "
            else:
                duration = " %s" % self.get_duration(lastrec[1],lastrec[2])
                status = "Last task: "
            if self.optlist.no_status:
                status = ""
            if self.optlist.no_duration:
                duration = ""
            
            print "%s%s%s" % (status, lastrec[0], duration)
        else:
            raise PunchCommandError       
...
"""
        ...
        parser.add_option('--no-status',action="store_true",default=False)
        parser.add_option('--no-duration',action="store_true",default=False)
